...
const (
	imageURL string = "https://bit.ly/2QGPDkr"
)

// ContentAwareResize resizes image using caire.
...
func main() {
	...
	http.HandleFunc("/occupymars", func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Query().Get("resize") > "" {
			resized, err := ContentAwareResize(imageURL)
			if err != nil {
			...
			fmt.Fprintf(w,
				...
				<html>
					...
					<img src="%s" />
					<br/>
					<a href="?resize=1">Resize using Seam Carving</a>
				</html>
				`, imageURL)
		}
	})
